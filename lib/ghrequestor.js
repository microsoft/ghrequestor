...
class GHRequestor {
  ...
  getAll(target, result = [], callback = null) {
    ...
    self.get(target, (err, response, body) => {
      ...
      if (err || response && response.status >= 300) {
        err = err || new Error('Non-2** response received')
   ...
   * If a callback is not supplied, a promise is returned. The promise will be resolved with the
   * response on success or rejected with an error object. Note that responses with status >=300 are not

...
class GHRequestor {
  ...
  static defaults(options) {
    return new RequestorTemplate(options);
  }
  ...
  static flattenResponses(responses, supplier) {
    ...
    return Q.all(chunks).then(resolvedChunks => {
      return resolvedChunks.reduce((result, element) => {
...
class RequestorTemplate {
  ...
  parseLinkHeader(header) {
    return GHRequestor.parseLinkHeader(header);
  }
}
...
class RequestorAction {
  ...
  _get(target, callback = null) {
    ...
    if (this.options.etags) {
      const etag = this.options.etag[this.activity.length - 1];
      if (etag) {

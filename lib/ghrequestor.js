...
class RequestorAction {
  ...
  getAll(target, callback = null) {
    this._initialize();
    const self = this;
    self.options.winstonLogger.info(`GetAllStarted: ${target}`);
  ...
  _getAll(target, callback = null) {
    ...
    self._get(target, (err, response, body) => {
      ...
      if (err) {
        self.options.winstonLogger.error(`GetAllError: ${target}, Message: ${err.toString()}`);
      ...
      if (response && (response.statusCode >= 300 && response.statusCode !== 304)) {
        self.options.winstonLogger.error(`GetAllResponseFail: ${target}, Status: ${response.statusCode}, Body:${response.statusMessage}`);
      ...
      if (response.headers.link) {
        ...
        if (links.next) {
          self.options.winstonLogger.info(`GetAllResponseNextPage: ${links.next.url}`);
  ...
  get(target, callback = null) {
    this._initialize();
    this.options.winstonLogger.info(`GetStarted: ${target}`);
  ...
  _get(target, callback = null) {
    ...
    options.retryStrategy = RequestorAction._retryStrategyWrapper(target, options.retryStrategy);

    request.get(target, options, (err, response, body) => {
      options.retryStrategy = actualRetry;
      if (response) {
        self.options.winstonLogger.debug(`GetResponseReceived: ${target}, Attempt: ${response.attempts}, Status: ${response.statusCode}`);
        activity.attempts = response.attempts;
        response.activity = self.activity;
      }
      if (err || !response) {
        err = err || new Error(response.statusMessage);
        self.options.winstonLogger.error(`GetError: ${target}, Status: ${err}`);
        err.response = response;
        err.activity = self.activity;
      ...
      if (response.statusCode >= 300) {
        self.options.winstonLogger.error(`GetFailedResponse: ${target}, StatusCode: ${response.statusCode}, Error:${response.statusMessage}`);
      ...
      if (self.options.delayOnThrottle && remaining < self.options.tokenLowerBound) {
        ...
        if (self.options.mode !== 'test') {
          self.options.winstonLogger.info(`GetTokenDelayStarted: ${target}, length: ${toSleep}, remaining: ${remaining}, reset: ${reset}`);
          return setTimeout(() => {
            return callback ? callback(err, response, body) : deferred.resolve(response);
          }, toSleep);
        }
  ...
  static _retryStrategyWrapper(target, cb) {
    return function (error, response, body) {
      if (error) {
        this.options.winstonLogger.info(`GetRetryStrategy target: ${target}, error: ${error.toString()}`)
      } else {
        this.options.winstonLogger.info(`GetRetryStrategy target: ${target}, response status: ${response.statusCode}, message: ${response.statusMessage}`);
      }
